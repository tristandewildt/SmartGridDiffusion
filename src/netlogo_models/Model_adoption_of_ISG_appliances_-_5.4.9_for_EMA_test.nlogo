...
to setup_adopter_properties
  ...
  ;A_S_L_valuation_of_savings_bounded_rational
  ; in contrary to the valuation of savings for full rational consumers, this is not about the savings by about the satisfaction about savings
...
to update_status_product
  ...
  while [g < number_of_appliance_switched_on][
    ifelse (runresult (word "Switch_appliance_"(g + 1))) = False [set g g + 1][
      ...
      [run (word "set " "ISG_appliance_part_"(g + 1)"_costs" " ((item 0 ISG_appliance_part_"(g + 1)")*((ticks - Start_year_introduction_smart_grid_Netherlands + 1) / (1))^(log (Learning_rate_appliances) 2))")]
      set g g + 1]]
end
